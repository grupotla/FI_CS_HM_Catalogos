...
class CarriersController extends Controller
{
	...
	public function accessRules()
	{
		...
		return ContactosUsersMenu::ControllerActionsSet2( 13 );
		/*
		return array(
			...
			if($model->save()) {
				...
				if ($this->asDialog) {
	                ...
	                if (window.parent.$.fn.yiiGridView)
	                	window.parent.$.fn.yiiGridView.update('carriers-grid');
	                ");
	                Yii::app()->end();
				} else {

			$this->redirect(array(Yii::app()->session['p']['update'] == 1 ? 'update' : 'modify','id'=>$model->carrier_id	));
	...
	{
		$dataProvider=new CActiveDataProvider('Carriers');
	...
	{
		$model=Carriers::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='carriers-form')
		{
	...
	public function actionGenerateExcel()
	{
		...
		$criteria = array();
		if(isset(Yii::app()->session['Carriers_records']))
			$criteria['criteria'] = Yii::app()->session['Carriers_records'];
		$criteria['pagination'] = array('pageSize'=>Carriers::model()->count());
		$model = new CActiveDataProvider('Carriers',$criteria);

		Yii::app()->request->sendFile('Carriers_'.date('YmdHis').'.xls',
    ...
    public function actionGeneratePdf()
	{
		...
		$html2pdf = Yii::app()->ePdf->HTML2PDF('P','A4','en',true,'UTF-8',array(5,5,5,5));
       	$html2pdf->pdf->SetTitle('PDF_Carriers');
       	$html2pdf->pdf->SetDisplayMode('fullpage');
        $html = $this->renderPartial('reportPdf', array('model'=>$model,'title'=>'Carriers'), true);
        $html2pdf->WriteHTML($html);
        $html2pdf->Output('Carriers_'.date('YmdHis').'.pdf');
	}
